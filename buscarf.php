...
class buscarf extends funciones
{
...
function getFactura($filtro="",$valor="")
{
	...
	if($filtro!=="" && $valor!=="")
	{
		...
		$qry = "SELECT * FROM factura WHERE $filtro = '$valor' ORDER BY ID;";
	}
    ...
    while($row = $STATEMENT->fetch_array())
        {
            ...
            echo "<a href='delf.php?from=buscarf.php&id=".$row['ID']."'>-Eliminar</a>";
            echo "<a href='creardoc.php?id=".$row['ID']."'>-Descargar</a>";           
            if($row['Estado']==="PENDIENTE")
            {
                echo "-<a href='".$_SERVER['PHP_SELF']."?accion=pagar&id=".$row['ID']."'>
                Pagar</a></td>";
            }
            else
            {
