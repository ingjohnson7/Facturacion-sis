...
header("Content-Disposition: attachment; Filename=Factura GJ.E.I. SRL.doc");

include_once("funciones.php");
$F = new funciones();
$C = $F->con();
$STM = $C->query("SELECT * FROM factura_i WHERE ID = ".$_GET['id'].";");
...
{
	$ID = $_GET['id'];
	$qry = "SELECT SUM(Importe) AS Total FROM detallei WHERE ID_factura = $ID;";
    $STM = $GLOBALS['C']->query($qry);
    $fila = $STM->fetch_array();
    $subt = $fila['Total'];
    return $GLOBALS['F']->FIX($subt);
}//end totales
...
<tr>
	...
	<td align="left" colspan="2">
		TEL: 809-318-7680, 849-227-2425  RNC: 131191568  
	</td>
	<tr>
	<td colspan="2">
